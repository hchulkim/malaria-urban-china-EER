...
coef_tbl[, .(year, estimate, std.error, conf.low, conf.high)] %>%
  kable(
    format = "latex", booktabs = TRUE, linesep = "", digits = 3,
    format.args = list(scientific = FALSE),
    col.names = c("Year", "Estimate", "Std. Error", "95% CI Low", "95% CI High")
  ) |>
  save_kable(here("output", "tables", "author_appendix_table_a1.tex"))
...
ggplot(coef_tbl, aes(x = year, y = estimate)) +
  ...
  labs(
    x = "Year", y = "MSMRelative coefficient (β)",
...
datasummary(`Urban share` + `Industry share` + `Log manufacturing output per capita` + `Log income per capita` + `Han share` + `Natural rate of increase` + `Child share` + `In-migration share` + `Malaria suitability(SD)` ~ Mean + SD + Min + Max + N,
  data = county_data_renamed,
...
texreg(list(res1, res2, res3, res4, res5, res6),
  digits = 3,
