...
xi: reg    r3_UrbanShare1990  MSMSD    $CrossSection   	,  cluster(Grid2) // variable is in percentage
estimates store j1
estadd local "Allcontrols"="Yes"  , replace : j1
xi: reg    r3_UrbanShare2010  MSMSD       $CrossSection  	   ,  cluster(Grid2)  // variable is in percentage
estimates store j2
estadd local "Allcontrols"="Yes"  , replace : j2
xi: reg    r3_ManufacturingShare1990  MSMSD   $CrossSection  	,  cluster(Grid2) // variable is in percentage
estimates store j3
estadd local "Allcontrols"="Yes"  , replace : j3

esttab  j* ///
	using "Table4rob_8b.tex" ///	
	, nogap fragment eqlabels(none)  nomtitles  replace  star(* 0.1 ** 0.05 *** 0.01) nonotes ///
	keep(MSMSD) ///
	nolines nonumbers ///
	substitute("\sym{*}" "*" "\sym{**}" "**" "\sym{***}" "***") ///	
...
xi: reg    r3_Seats1893 MSMSD     $CrossSection   	,  cluster(Grid2) // variable is in percentage
estimates store j1
estadd local "Allcontrols"="Yes"  , replace : j1
xi: reg    r3_UrbanShare1893  MSMSD       $CrossSection  	   ,  cluster(Grid2)  // variable is in percentage
estimates store j2
estadd local "Allcontrols"="Yes"  , replace : j2
xi: reg    r3_Hierarchy1893  MSMSD   $CrossSection  	,  cluster(Grid2) // variable is in percentage
estimates store j3
estadd local "Allcontrols"="Yes"  , replace : j3

esttab  j* ///
	using "Table3rob_8b.tex" ///	
	, nogap fragment eqlabels(none)  nomtitles  replace  star(* 0.1 ** 0.05 *** 0.01) nonotes ///
	keep(MSMSD) ///
	nolines nonumbers ///
	substitute("\sym{*}" "*" "\sym{**}" "**" "\sym{***}" "***") ///	
